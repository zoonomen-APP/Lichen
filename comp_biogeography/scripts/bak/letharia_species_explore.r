...
COL_COLUMBIANA <- "#00FFFF"   # cyan  (same family as genus plot)
COL_VULPINA    <- "#FFD700"   # gold  (distinct, still readable on plasma)
...
## ---- PLOTS 2-4: Species level ----------------------------------------------
##
## occ_counts_waor has no species column -- it was built at genus level.
## Species-level plots (columbiana, vulpina) need a separate query from the
## CLH database.  To be added once plot 1 is confirmed working.

cat("Plots 2-4: Skipped -- species-level data not yet in RDS.\n")
cat("  Next step: query CLH DB for L. columbiana and L. vulpina hex counts.\n\n")
