...
## ---- PLOT 4: Both species together ----------------------------------------
##
## columbiana = cyan, vulpina = gold (#FFD700)
## Pre-compute alpha per species to avoid ggplot2 dual-scale limitation.

cat("Plot 4: Both species together...\n")

if (!is.null(col_df) && !is.null(vul_df) &&
    nrow(col_df) > 0 && nrow(vul_df) > 0) {

  rescale_alpha <- function(log_n_vec, range = c(0.15, 0.95)) {
    rng <- range(log_n_vec, na.rm = TRUE)
    if (diff(rng) == 0) return(rep(range[2], length(log_n_vec)))
    range[1] + (log_n_vec - rng[1]) / (rng[2] - rng[1]) * diff(range)
  }

  occ_sf_col$alpha_val <- rescale_alpha(occ_sf_col$log_n)
  occ_sf_vul$alpha_val <- rescale_alpha(occ_sf_vul$log_n)

  p4 <- base_plot("Letharia columbiana \u2022 L. vulpina: The Void") +

    ## vulpina first (underneath) -- gold
    geom_sf(data = occ_sf_vul,
            fill  = "#FFD700",
            alpha = occ_sf_vul$alpha_val,
            color = NA) +

    ## columbiana on top -- cyan
    geom_sf(data = occ_sf_col,
            fill  = "#00FFFF",
            alpha = occ_sf_col$alpha_val,
            color = NA) +

    ## Manual legend -- SE Oregon high desert, well inside map
    annotate("rect", xmin = -119.5, xmax = -118.5,
             ymin = 42.1, ymax = 42.5,
             fill = "#00FFFF", alpha = 0.85) +
    annotate("rect", xmin = -119.5, xmax = -118.5,
             ymin = 41.5, ymax = 41.9,
             fill = "#FFD700", alpha = 0.85) +
    annotate("text", x = -118.3, y = 42.3,
             label = "L. columbiana", color = "white",
             hjust = 0, size = 3.5) +
    annotate("text", x = -118.3, y = 41.7,
             label = "L. vulpina", color = "white",
             hjust = 0, size = 3.5)

  ggsave(file.path(out_dir, "letharia_04_both_void.png"),
         plot = p4, width = 12, height = 8, dpi = 300)
  cat("  Saved: letharia_04_both_void.png\n\n")

} else {
  cat("  Skipped: one or both species missing\n\n")
}
